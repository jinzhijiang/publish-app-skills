...
## 用 `query` 间接判断「有没有待审提交」（2026-09-03 实测）

小米**未开放审核状态查询接口**，但 `/dev/query` 的三个布尔位可以当状态信号用：
...
| `create` | `false` | `false` |
| `packageInfo.onlineVersionCode` | 在架版本 | **不变** |

`push` 成功后这两个 `true` 会翻成 `false`——平台不允许在已有待审提交时再次提交。
等审核结束（通过或驳回）才会翻回 `true`。

所以：
- **`updateVersion=false` = 有一笔提交挂在审核里**，此时再 `push` 会被拒；
- **判断是否已上架看 `onlineVersionCode`**，它在提交后不变，只有审核通过发布才更新；
- 驳回原因仍然只能登开放平台网页看，接口拿不到。
